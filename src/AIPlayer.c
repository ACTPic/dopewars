...
void AIPlayerLoop(struct CMDLINE *cmdline)
{
	...
	while (1) {
		...
		SetSelectForNetworkBuffer(netbuf, &readfs, &writefs, NULL,
					  &MaxSock);
		...
		datawaiting =
		    RespondToSelect(netbuf, &readfs, &writefs, NULL,
				    &DoneOK);
		...
		if (datawaiting && netbuf->status == NBS_CONNECTED) {
			QuitRequest = FALSE;
			while ((msg =
				GetWaitingPlayerMessage(AIPlay)) != NULL) {
...
static void HandleCombat(Player * AIPlay, gchar * Msg)
{
	...
	if (HaveAbility(AIPlay, A_NEWFIGHT)) {
		ReceiveFightMessage(Msg, &AttackName, &DefendName,
				    &DefendHealth, &DefendBitches,
...
int HandleAIMessage(char *Message, Player * AIPlay)
{
	...
	Handled =
	    HandleGenericClientMessage(From, AI, Code, AIPlay, Data, NULL);
	switch (Code) {
	case C_ENDLIST:
	...
	default:
		if (!Handled)
			g_message("%s^%c^%s%s\n", GetPlayerName(From),
				  Code, GetPlayerName(AIPlay), Data);
		break;
	}
	return 0;
}
	...
	do {
		...
		if (Highest >= 0) {
			...
			if ((Turn >= 59 || MaxProfit > 0) && Num > 0) {
				...
				AIPlay->CoatSize += Num;
				AIPlay->Cash +=
				    Num * AIPlay->Drugs[Highest].Price;
				text =
				    g_strdup_printf("drug^%d^%d", Highest,
						    -Num);
				SendClientMessage(AIPlay, C_NONE,
 ...
 */
void AIHandleQuestion(char *Data, AICode AI, Player * AIPlay,
		      Player * From)
{
	char *Prompt;
	...
	case C_MEETPLAYER:
		...
		break;
	case C_ASKSEW:
		AISendAnswer(AIPlay, From,
			     AIPlay->Health < MINSAFEHEALTH ? "Y" : "N");
		break;
	default:
		AISendAnswer(AIPlay, From, "N");
		break;
...
{
	g_print(_
		("This binary has been compiled without networking support, "
